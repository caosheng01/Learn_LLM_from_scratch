...
举个例子详细说一下Patch Embedding过程。输入图片大小为224x224，将图片分为固定大小的patch，patch大小为16x16，则每张图像会生成224x224/16x16=196个patch，即输入序列长度为196，每个patch维度3x16x16=768，线性投射层的维度为768xN (N=768)，因此输入通过线性投射层之后的维度依然为196x768，即一共有196个token，每个token的维度是768。这里还需要加上一个特殊字符CLS，因此最终的Tokens数是197。下图详细展示了这个Patch Embedding的过程。
...
其实BEiT-3最值得学习的地方就是预训练的思路。我们先了解一下模型训练通用方法，主要就是聚焦在下面这两个方面：

1. 模型训练方法，比如有监督微调(SFT)，强化学习(RL)等；
2. 训练数据

对于训练方法，我们讨论的比较多，暂时按下不表。我们来看一下训练数据，对于训练模型来说，数据质量越高越好，数据规模越大越好。这个两方面需求其实是相互矛盾的，质量越高，就需要人工标记。这样的采用人工标注的方式，规模一定受限。在CNN时代（以处理图像为主），业界流行的方式就是采用人工标注来产生数据，诞生了很多很有影响力的数据集，比如ImageNet。但是，到了NLP阶段，特别是LLM流行起来后，Scalling Law成为很多模型的不二选择。这样数据规模就成了瓶颈。摆在业界面前的难题就是需要在数据规模和数据质量中间取一个平衡点。这样，自监督学习(Self-survised Learning)就孕育而生了，说白了，思路也很简单——让机器代替人来对数据做标注。用自监督学习解决了训练规模的问题后，离激活Scalling Law又迈进一步。
接着我们回到模型训练方法来，要想激活Scalling Law，BEiT-3还需要在模型训练方法论有所创新。上个章节提到VLMo的一个痛点就是ITC+ITM+MLM的联合训练太难了，不利于激活Scalling Law。现在我们就来看BEiT-3是如何简化这个Loss function。一般来说，简化原来的方法，要么提出一种全新的方法代替原方法，要么在原来基础上做减法，而BEiT-3采用了后一种方法。
只要分析一下ITC+ITM，其实这两种方法还相互依赖，中间还穿插着一个hard negitive sampling。在反观NLP领域，也没有相对应的方法，换句话说，ITC+ITM也不大适合纯文字的领域。这样就剩下来MLM了，不管是应付Image还是Text类型的数据都行，第一感觉，有戏！再回头看看ITC+ITM工作的比较好的地方，就是处理输入类型为Text-Image Pair这类数据。如果我们能用MLM的方法，能处理Text-Image Pair类型数据，那么就可以放心大胆的抛弃ITC+ITM了。接着，我们来看一下BEiT团队是如何解决这个问题的。
...
1. **核心逻辑**
   ...
   | 传统目标        | MDM 替代方式                                                                                    |
   | --------------- | ----------------------------------------------------------------------------------------------- |
   | ITC（特征对齐） | 通过掩码token的上下文预测，让图文token在同空间自然对齐，无需额外对比约束                        |
   | ITM（匹配判断） | 模型能通过恢复掩码token的合理性，隐式判断图文对是否匹配，噪声对匹配的负面影响被掩码任务自然过滤 |
   | 多任务协同      | 单损失函数简化训练流程，避免多目标冲突，降低训练成本，提升收敛效率                              |
2. **效果验证**

   * 预训练后可通过简单微调适配跨模态检索、VQA、图像描述等任务：检索任务直接取图文token序列的全局特征做相似度计算；VQA/图像描述任务通过任务头转换输出格式，无需重构模型。
   * 在COCO、Flickr30K等数据集的跨模态检索任务中，性能优于传统ITC/ITM多任务训练模型，验证了MDM的有效性。
3. **总结**


| 维度     | BEiT-3（MDM）              | 传统ITC+ITM方法                 |
| -------- | -------------------------- | ------------------------------- |
| 核心任务 | 统一掩码恢复，单损失函数   | 对比学习+匹配判断，多损失函数   |
| 模态交互 | 双向深度融合，细粒度关联   | 全局特征对齐+二分类匹配，交互浅 |
| 训练效率 | 单次前向传播，成本低       | 多次前向，算力/显存开销大       |
| 任务适配 | 单架构覆盖全任务，微调灵活 | 需针对检索/匹配任务单独设计分支 |

### 写在最后

我们将上一个章节讨论的理想MLLM架构和这一章节讨论的BEiT-3模型实现的做一个对比，如下图所示：

![MLLM_BEiTv3_Overview.svg](../images/MLLM_BEiTv3_Overview.svg)

由此可见，BEiT-3几乎就是我们理想中的MLLM。至此，一堆做多模态模型的“卷王”们，在短短2年时间，到2023年底，就卷出了大一统的MLLM模型。
按照时间线
