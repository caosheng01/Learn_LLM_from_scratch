...
在深入学习NLP前，我们需要先做一些技术准备。NLP的技术基础方面，我认为主要是这两部分：
...
因为我们聚焦在大语言模型的学习，这部分不详细讨论了。直接写上我觉得学习CNN最有用的心得。
...
单词的位置和顺序是任何语言的重要部分。它们定义了语法，从而确定了句子的实际语义。递归神经网络（RNNs）本质上考虑了单词的顺序；它们以顺序方式逐词解析句子。这将把单词的顺序整合到RNNs的主干中。但是，Transformer架构放弃了递归机制，转而采用多头自注意力机制。避免RNNs的递归方法将大大加快训练时间。理论上，它可以捕获句子中更长的依赖关系。
