...
#### Zero-Shot，One-Shot和Few-Shot Learning

计算机是个应用学科，实践是检验真理的唯一标准。也就是说，衡量理论或算法的标准，必须以实际效果为准。大量实验表明，GPT-2的无监督学习（就是Zero-Shot learning）能力还有很大的提升空间，GPT-2在很多不同的task中，有些表现良好，但在某些领域仍然表现欠佳，比如summarization、QA等，最终效果没有超过BERT。问题的根源就在Zero-Shot learning实际效果没有理论的好。GPT-3就把这个Zero-Shot换成了Few-Shot Learning（也称ICT），也就是又找人对学习样本做了标注。

> OpenAI的GPT发展史，从模型训练的角度看，实际上就是从Zero-Shot（GPT-2）到Few-Shot（GPT-3）再到HFRL（InstructGPT/ChatGPT）。

这一章节，我们来聊聊**无样本学习**（Zero-Shot Learning），**单样本学习**（One-Shot Learning）和**少量样本学习**（Few-Shot Learning）。
...
网上很多文章介绍ICL时，直接等同于Few-Shot Learning。大方向上看，这种论点也是可以接受的。笔者认为ICL应该更加像Zero-Shot，One-Shot和Few-Shot三者的集合。我们抛开概念上的咬文嚼字，真正需要学习的时，ICL和传统二阶段训练中的微调（Fine tuning）的区别。细心的读者，可能记得笔者在前面章节举了一个例子，如何把一个普通项目级别的User svc变成一个强大的平台级别的通用User Svc的做法。现在我们就把这个坑填了。
